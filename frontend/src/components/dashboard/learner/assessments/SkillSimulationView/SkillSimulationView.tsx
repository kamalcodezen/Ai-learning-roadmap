...
export default function SkillSimulationView({
  skill,
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["readiness", session?.user?.id],
      });
      queryClient.invalidateQueries({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4 text-center max-w-md mx-auto">
        ...
        <p className="text-sm text-muted-foreground">
          You have not completed an evaluation for {skill} yet. Start a new
          simulation below.
        </p>
        <DashboardButton text="Start Simulation" onClick={() => setIsReviewMode(false)} />
      </div>
    );
  }
  ...
  if (isSimulationError || !simulationData) {
    return (
      <div className="flex flex-col items-center justify-center h-64 space-y-4 text-center max-w-md mx-auto">
        ...
        <p className="text-sm text-muted-foreground">
          Unable to generate your personalized assessment right now. Please
          click below to retry.
        </p>
        <DashboardButton text="Retry" onClick={() => refetchSimulation()} />
      </div>
    );
  }
  ...
  const isCurrentStageAnswered = () => {
    switch (currentStage) {
      case 0:
        return understandAnswer.trim().length > 0;
      case 1:
        return debugAnswer.trim().length > 0;
      case 2:
        return effectiveCode.trim().length > 10;
      case 3:
        return explainAnswer.trim().length > 10;
      default:
        return false;
    }
  };
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs text-muted-foreground font-medium">
            <span>
              Step {currentStage + 1} of 4: {stageTitles[currentStage].title}
            </span>
