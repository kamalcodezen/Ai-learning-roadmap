...
import { CheckCircle2, AlertTriangle, ArrowRight, RotateCcw, Target, Sparkles, Code2, Bug, BookOpen, MessageSquareText } from "lucide-react";
    ...
    <div className="flex flex-col gap-8 pb-12 animate-in fade-in duration-500 max-w-4xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-border/60 pb-6">
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-3xl font-bold text-foreground">{result.skill}</h1>
            {result.targetRole && (
          ...
          <p className="text-sm text-muted-foreground">
            Completed on {new Date(result.completedAt).toLocaleDateString()} with authentic performance scoring.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            ...
            className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium hover:brightness-110 transition-all flex items-center gap-2"
          >
        ...
        <CardContent className="p-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center md:text-left">
              <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1 rounded-full border ${scoreColor}`}>
                {isPassing ? <CheckCircle2 className="w-3.5 h-3.5" /> : <AlertTriangle className="w-3.5 h-3.5" />}
                {isPassing ? "Mastery Demonstrated" : result.overallScore >= 50 ? "Foundational Proficiency" : "Active Skill Gap"}
              </span>
              <h2 className="text-2xl font-bold text-foreground">
                {isPassing ? "You demonstrated strong practical understanding!" : "Continued practice recommended."}
              </h2>
            ...
            <div className="flex flex-col items-center justify-center p-6 rounded-2xl bg-card-soft border border-border/70 shrink-0 min-w-[160px]">
              ...
              <span className={`text-5xl font-extrabold tracking-tight ${
                isPassing ? "text-green-500" : result.overallScore >= 50 ? "text-amber-500" : "text-rose-500"
              }`}>
                {result.overallScore}%
              </span>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-primary" /> Evaluation Breakdown by Stage
        </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm font-semibold">
                  <BookOpen className="w-4 h-4 text-blue-500" /> 1. Understand & Concepts
                </div>
                <span className="font-bold text-sm">{result.stageBreakdown.understand}%</span>
              </div>
              ...
              <p className="text-xs text-muted-foreground">Fundamental principles, runtime mechanics, and theory.</p>
            </CardContent>
          </Card>
          ...
          <Card className="border border-border/70">
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm font-semibold">
                  <Bug className="w-4 h-4 text-amber-500" /> 2. Debugging & Analysis
                </div>
                <span className="font-bold text-sm">{result.stageBreakdown.debug}%</span>
              </div>
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                <div
              ...
              <p className="text-xs text-muted-foreground">Identifying subtle code bugs and implementing root-cause fixes.</p>
            </CardContent>
          </Card>
          ...
          <Card className="border border-border/70">
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm font-semibold">
                  <Code2 className="w-4 h-4 text-emerald-500" /> 3. Code Implementation
                </div>
                <span className="font-bold text-sm">{result.stageBreakdown.code}%</span>
              </div>
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                <div
              ...
              <p className="text-xs text-muted-foreground">Writing robust, syntactically correct code following standards.</p>
            </CardContent>
          </Card>
          ...
          <Card className="border border-border/70">
            <CardContent className="p-5 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm font-semibold">
                  <MessageSquareText className="w-4 h-4 text-purple-500" /> 4. Technical Explanation
                </div>
                <span className="font-bold text-sm">{result.stageBreakdown.explain}%</span>
              </div>
              <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
                <div
              ...
              <p className="text-xs text-muted-foreground">Articulating architectural tradeoffs in your own words.</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <div className="p-6 rounded-xl bg-card border border-border flex flex-col sm:flex-row items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Study curated resources on your active Learning Path to reinforce this skill.
          </p>
        </div>
        <Link
          href={`/dashboard/learner/learning-path?skill=${encodeURIComponent(result.skill)}`}
          className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:brightness-110 transition-all flex items-center gap-1.5 shrink-0"
        >
