...
export default function JobRealityContent() {
      ...
      return (
        <div className="flex flex-col items-center justify-center p-12 text-center animate-in fade-in">
          ...
          <p className="text-muted-foreground max-w-md mb-6">
            We need to know your target career before we can fetch the job
            market reality.
          </p>
    ...
    return (
      <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed border-border rounded-xl">
        ...
        <p className="text-muted-foreground mb-6">
          We could not fetch real-time market data from the external provider at
          this moment.
        </p>
  ...
  const jobData = (
    dataObj.success && dataObj.data ? dataObj.data : data
  ) as JobRealityData;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-2">
            Real-time market demand and expectations for{" "}
        ...
        <div className="text-xs text-muted-foreground flex items-center gap-1.5 bg-muted/50 px-3 py-1.5 rounded-full border border-border/50">
          ...
          Data via {jobData.source.provider} • Updated{" "}
          {formatDistanceToNow(new Date(jobData.source.fetchedAt))} ago
        </div>
      </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <BarChart3
                className={`w-5 h-5 ${jobData.market.demandLevel === "High" ? "text-green-500" : jobData.market.demandLevel === "Medium" ? "text-amber-500" : "text-muted-foreground"}`}
              />
            </div>
            <h3 className="text-3xl font-bold">{jobData.market.demandLevel}</h3>
            <p className="text-xs text-muted-foreground mt-2">
              Current provider estimation
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            ...
            <h3 className="text-3xl font-bold">
              {jobData.market.jobCount !== null
                ? jobData.market.jobCount
                : "Unavailable"}
            </h3>
            <p className="text-xs text-muted-foreground mt-2">
              Recent listings analyzed
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <TrendingUp
                className={`w-5 h-5 ${jobData.market.trend === "Growing" ? "text-green-500" : "text-muted-foreground"}`}
              />
            </div>
            <h3 className="text-3xl font-bold">
              {jobData.market.trend || "Unavailable"}
            </h3>
            <p className="text-xs text-muted-foreground mt-2">
              Based on listing frequency
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <DashboardCard className="h-full">
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                How your current abilities map to what employers are asking for
                today.
              </p>
            </CardHeader>
            <CardContent>
