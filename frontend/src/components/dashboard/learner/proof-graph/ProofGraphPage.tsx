...
import { getGamificationProfile, getSkillTree } from "@/src/lib/api/learner/gamification";
...
export default function ProofGraphPage() {
  ...
  const [activeTab, setActiveTab] = useState<"proof-graph" | "skill-tree" | "achievements">("proof-graph");
  ...
  const getBadgeIcon = (badgeIcon: string) => {
    switch (badgeIcon) {
      ...
      case "Flag": return <Flag className="w-6 h-6 text-blue-500" />;
      case "FolderGit2": return <FolderGit2 className="w-6 h-6 text-emerald-500" />;
      case "ShieldCheck": return <ShieldCheck className="w-6 h-6 text-teal-500" />;
  ...
  return (
    <div className="flex flex-col gap-8 pb-12 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Skill Tree & Proof Graph</h1>
          <p className="text-muted-foreground">Trace verified competence, unlock mastery nodes, and earn career badges.</p>
        </div>
      ...
      <DashboardCard className="bg-primary/5 border-primary/20">
        <CardContent className="p-6 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="space-y-1 text-center md:text-left">
            <p className="text-sm text-muted-foreground">Analyzing Career Target</p>
            ...
            {gamificationData?.levelInfo && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground mt-1">
                <span className="font-semibold text-foreground">Level {gamificationData.levelInfo.level}:</span>
          ...
          {gamificationData?.levelInfo && (
            <div className="flex-1 max-w-xs w-full">
              <div className="flex justify-between text-xs font-semibold mb-1">
                <span className="flex items-center gap-1 text-primary"><Zap className="w-3.5 h-3.5" /> {gamificationData.totalXp ?? 0} XP</span>
                <span className="text-muted-foreground">{gamificationData.levelInfo.nextLevelXp} XP (Lvl {gamificationData.levelInfo.level + 1})</span>
              </div>
              <div className="w-full bg-muted rounded-full h-2.5 overflow-hidden">
                <div
                  className="bg-primary h-2.5 rounded-full transition-all duration-500"
                  style={{ width: `${gamificationData.levelInfo.progressPercent ?? 0}%` }}
                />
              </div>
            </div>
          )}
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <Card className="p-4 border-green-500/20 bg-green-500/5">
                  ...
                  <p className="text-2xl font-bold text-green-500">{skillTreeData.masteredCount}</p>
                </Card>
                <Card className="p-4 border-amber-500/20 bg-amber-500/5">
                  <p className="text-xs text-muted-foreground font-semibold">IN PROGRESS</p>
                  <p className="text-2xl font-bold text-amber-500">{skillTreeData.inProgressCount}</p>
                </Card>
                <Card className="p-4 border-blue-500/20 bg-blue-500/5">
                  <p className="text-xs text-muted-foreground font-semibold">AVAILABLE</p>
                  <p className="text-2xl font-bold text-blue-500">{skillTreeData.availableCount}</p>
                </Card>
                <Card className="p-4 border-muted">
                  <p className="text-xs text-muted-foreground font-semibold">LOCKED</p>
                  <p className="text-2xl font-bold text-muted-foreground">{skillTreeData.lockedCount}</p>
                </Card>
              </div>
                  ...
                  >
                    <CardContent className="p-5 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {node.status === "LOCKED" && <Lock className="w-3 h-3 inline mr-1" />}
                          {node.status}
                        </span>
                      </div>
                        ...
                        <div className="w-full bg-muted rounded-full h-1.5">
                          <div
                            className={`h-1.5 rounded-full ${
                              node.masteryScore >= 70 ? "bg-green-500" : "bg-amber-500"
                            }`}
                            style={{ width: `${Math.min(100, node.masteryScore)}%` }}
                          />
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground pt-1 border-t border-border/50">
                        <div>Knowledge: <span className="font-semibold text-foreground">{node.knowledgeScore}%</span></div>
                        <div>Evidence: <span className="font-semibold text-foreground">{node.evidenceScore}%</span></div>
                      </div>
                    </CardContent>
                  </Card>
                ))}
        ...
        <div className="w-full space-y-6">
          {gamificationData && (
            <>
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xl font-bold">Earned Badges ({gamificationData.unlockedCount ?? 0}/{gamificationData.totalAchievementsCount ?? 0})</h3>
                  <p className="text-sm text-muted-foreground">Unlock career achievements through real proof and milestone accomplishments.</p>
                </div>
              </div>
